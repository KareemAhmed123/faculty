...
@Repository
public interface CourseRepositoryRepository extends JpaRepository<Course, UUID> {
	...
	/*
	 * @Query(value =
	 * "update Course c set c.title = ?1, c.courseId = ?2, c.prerequisite = ?3 where c.id = ?4"
	 * ) public void update(@Param("title") String title , @Param("courseId") int
	 * courseId, @Param("prerequisite") Course prerequisite, @Param("id") Long id);
	 */
